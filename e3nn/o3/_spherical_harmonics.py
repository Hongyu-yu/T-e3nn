    ...
    def __init__(
        ...
        if isinstance(irreps_out, o3.Irreps):
            ...
            for mul, (l, p, t) in irreps_out:
                if t != input_t**l:
                    raise ValueError(
                        f"irreps_out `{irreps_out}` passed to SphericalHarmonics asked for an output of l = {l} with TR "
                        f"t = {t}, which is inconsistent with the input t {input_t} — the output t should have been "
                        f"t = {input_t**l}"
                    )
                if p != input_p**l:
                    raise ValueError(
